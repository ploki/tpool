...
static void
ttask_pool_delete_func(ttask_pool *pool,
                       ttask *task)
{
}
...
ttask *
ttask_pool_get(ttask_pool *pool)
{
        ttask *task = NULL;
...
int
ttask_pool_put(ttask_pool *pool,
               ttask *task)
{
        int ret, rc;
        ...
        rc = pool->task_put_func(pool, task);
        if (rc) {
                ret = rc;
                goto end;
        }
...
static void
ttask_pool_stop(ttask_pool *pool)
{
        pthread_mutex_lock(&pool->task_lock);
...
static void
ttask_pool_wait_idle(ttask_pool *pool)
{
        pthread_mutex_lock(&pool->task_lock);
...
static void
ttask_pool_dtor(ttask_pool *pool)
{
...
int
ttask_pool_ctor(ttask_pool *pool,
                int n_workers)
{
        int ret;
        ...
        ret = pthread_attr_init(&pool->attr);
        if (ret)
                goto bad;
        ...
        ret = ttask_pool_set_workers(pool, n_workers);
        if (ret) {
...
ttask_pool *
ttask_pool_create(int n_workers)
{
        ...
        ret = ttask_pool_ctor(pool, n_workers);
        if (ret)
                goto bad;
...
int
ttask_pool_set_workers_min(ttask_pool *pool,
                           int n_workers_min)
{
        if (n_workers_min < 0)
                return -1;
...
int
ttask_pool_set_workers(ttask_pool *pool,
                       int n_workers_needed)
{
        int i, ret = 0;
